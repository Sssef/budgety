...
var budgetController = (function () {
    ...
    return {
        addItem:function (type, des, vl) {
            ...
            if (type === 'exp') {
                newItem = new Expense(ID, des, vl);
            } else if (type === 'inc') {
                newItem = new Income(ID, des, vl);
            }
        ...
        calculatePercents: function () {
            data.allItems.exp.forEach(function (cur) {
                cur.calcPercents(data.totals.inc);
            });
        },

        getPercents: function () {
            var allPerc = data.allItems.exp.map(function (cur) {
                return cur.getPercents();
            });
            return allPerc;
        },
...
var UIController = (function () {
    ...
    var formatNumber = function (num, type) {
        ...
        int = numSplit[0];
        if (int.length > 3) {
            int = int.substr(0, int.length - 3) + ',' + int.substr(int.length - 3, 3); //input 32110, output 32,110
        }
...
var controller = (function (budgetCtrl, UICtrl) {
    ...
    var updatePercents = function (arguments) {
        // 1. Calc percents
        budgetCtrl.calculatePercents();

        // 2. Read percents from budget controller
        var percents = budgetCtrl.getPercents();

        // 3. Update the UI with the new percents
    ...
    var ctrlAddItem = function () {
        ...
        if (input.descr !== "" && !isNaN(input.val) && input.val > 0 ) {
            ...
            newItem = budgetCtrl.addItem(input.type, input.descr, input.val);
            ...
            // 6.  Calc and update percents
            updatePercents();
        }
    };

    var ctrlDeleteItem = function (event) { //event = e
        ...
        if (itemID) {
            ...
            // 1. Delete the item from data scructure
